  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-md flex items-center justify-center p-0 sm:p-4">
      <div className="bg-[#1A1A1A] w-full sm:max-w-4xl sm:rounded-2xl rounded-t-3xl border border-neutral-800 text-white max-h-[95vh] sm:max-h-[85vh] flex flex-col sm:flex-row shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 relative">
        ...
        {/* Close button for desktop (top right of modal) */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-30 p-2 rounded-full bg-black/70 backdrop-blur-md text-white hover:bg-black transition hidden sm:flex items-center justify-center shadow-lg border border-white/10"
        >
          <X className="w-5 h-5" />
        </button>

        {/* Photo Section with Carousel (Left Side on Desktop) */}
        <div className="relative w-full sm:w-[240px] sm:h-[360px] aspect-[4/5] sm:aspect-auto bg-black flex-shrink-0">
          <img
          ...
          <button
            onClick={onClose}
            className="absolute top-4 right-4 z-20 p-2 rounded-full bg-black/60 backdrop-blur-md text-white hover:bg-black transition sm:hidden"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          {profile.photos.length > 1 && (
            <>
              <div className="absolute top-4 left-4 right-16 sm:right-4 z-10 flex space-x-1">
        ...
        {/* Right Column: Profile Details & Actions on the Right Side of Photo */}
        <div className="flex-1 flex flex-col overflow-hidden">
          
          {/* Scrollable Bio & Stats */}
          <div className="p-5 overflow-y-auto space-y-6 flex-1 max-h-[70vh] sm:max-h-[70vh]">
            ...
            <div className="hidden sm:flex items-center justify-between border-b border-neutral-800 pb-4 pr-12">
              ...
              <button
                ...
                className={`p-3 rounded-full backdrop-blur-md border transition ${
                  profile.isFavorite
                    ? 'bg-[#FFC107] text-[#121212] border-[#FFC107]'
                    : 'bg-[#252525] text-white border-neutral-700 hover:bg-neutral-800'
                }`}
              >
                <Star className="w-5 h-5 fill-current" />
              </button>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {profile.pronouns && (
                <div className="bg-[#222222] p-3 rounded-xl border border-neutral-800/80">
                  <span className="text-[11px] uppercase tracking-wider text-neutral-400 block">Pronouns</span>
                  <span className="font-bold text-white text-sm mt-0.5 block">{profile.pronouns}</span>
                </div>
              )}
              {profile.zodiac && (
                <div className="bg-[#222222] p-3 rounded-xl border border-neutral-800/80">
                  <span className="text-[11px] uppercase tracking-wider text-neutral-400 block">Zodiac</span>
                  <span className="font-bold text-white text-sm mt-0.5 block">{profile.zodiac}</span>
                </div>
              )}
              {profile.height && (
                <div className="bg-[#222222] p-3 rounded-xl border border-neutral-800/80">
              ...
              {profile.smoking && (
                <div className="bg-[#222222] p-3 rounded-xl border border-neutral-800/80">
                  <span className="text-[11px] uppercase tracking-wider text-neutral-400 block">Smoking</span>
                  <span className="font-bold text-white text-sm mt-0.5 block">{profile.smoking}</span>
                </div>
              )}
              {profile.drinking && (
                <div className="bg-[#222222] p-3 rounded-xl border border-neutral-800/80">
                  <span className="text-[11px] uppercase tracking-wider text-neutral-400 block">Drinking</span>
                  <span className="font-bold text-white text-sm mt-0.5 block">{profile.drinking}</span>
                </div>
              )}
              {profile.relationshipStatus && (
                <div className="bg-[#222222] p-3 rounded-xl border border-neutral-800/80">
                  <span className="text-[11px] uppercase tracking-wider text-neutral-400 block">Relationship</span>
                  <span className="font-bold text-white text-sm mt-0.5 block">{profile.relationshipStatus}</span>
                </div>
              )}
            ...
            {profile.socialLinks && Object.values(profile.socialLinks).some(Boolean) && (
              <div className="space-y-2">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-neutral-400">Social Links & Music</h3>
                <div className="flex flex-wrap gap-2">
                  {profile.socialLinks.instagram && (
                    <a
                      ...
                      className="px-3 py-1.5 rounded-xl bg-pink-500/15 border border-pink-500/30 text-pink-400 text-xs font-semibold flex items-center gap-1.5 hover:bg-pink-500/25 transition"
                    >
                      <span>📸</span>
                  ...
                  {profile.socialLinks.twitter && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="px-3 py-1.5 rounded-xl bg-blue-500/15 border border-blue-500/30 text-blue-400 text-xs font-semibold flex items-center gap-1.5 hover:bg-blue-500/25 transition"
                    >
                      <span>🐦</span>
                  ...
                  {profile.socialLinks.snapchat && (
                    <div className="px-3 py-1.5 rounded-xl bg-yellow-500/15 border border-yellow-500/30 text-yellow-400 text-xs font-semibold flex items-center gap-1.5">
                      <span>👻</span>
                      <span>{profile.socialLinks.snapchat}</span>
                    </div>
                  )}
                  {profile.socialLinks.spotify && (
                    <div className="px-3 py-1.5 rounded-xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 text-xs font-semibold flex items-center gap-1.5">
                      <span>🎧</span>
                      <span>{profile.socialLinks.spotify}</span>
                    </div>
                  )}
                </div>
              </div>
